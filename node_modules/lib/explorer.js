 ...
 //     uri += '?filter=enabled';
  ...
  get_connectioncount: function(cb) {
    if (settings.baseType === 'ripple') {
      request(request({
        method: 'POST',
        uri: 'http://'+settings.wallet.host+':'+settings.wallet.port+'/',
        json: true,
        body: {method: "peers"}
      }, function (err, response, body) {
        if (!body.result || body.result.peers)
          return false;
        return cb(body.result.peers.length);
      }));
    }
    else {
  ...
  get_blockcount: function(cb) {
    if (settings.baseType === 'ripple') {
      request(request({
        ...
        uri: 'http://'+settings.wallet.host+':'+settings.wallet.port+'/',
        json: true,
        body: { method: "ledger_current" }
      }, function (err, response, body) {
        return cb(body.result.ledger_current_index);
      }));
    }
    else {
  ...
  get_blockhash: function(height, cb) {
    if (settings.baseType === 'ripple') {
      request(request({
        ...
        if (body.result && body.result.ledger_hash) {
          return cb(body.result.ledger_hash);
        } else {
          console.log("Data not available. Data:", body);
          return cb(false);
        }
      }));
    }
  ...
  get_block: function(hash, cb) {
    if (settings.baseType === 'ripple') {
      request(request({
        ...
        if (body.result && body.result.ledger) {
          return cb(body.result.ledger);
        } else {
          console.log("Data not available. Data:", body);
          return cb(false);
        }
  ...
  get_account_balance: function(accountHash, ledgerHash, cb) {
    if (settings.baseType === 'ripple') {
      request(request({
        method: 'POST',
        uri: 'http://'+settings.wallet.host+':'+settings.wallet.port+'/',
        ...
        if (body.result && body.result.account_data) {
          return cb(+body.result.account_data.Balance);
        } else {
          console.log("Data not available. Data:", body);
          return cb(false);
        }
      ...
      throw 'Not supported for non-ripple based coins';
    }
  },
  ...
  get_supply: function(cb) {
    if (settings.baseType === 'ripple') {
      request(request({
        ...
        if (body.result && body.result.ledger && body.result.ledger.total_coins) {
          var supply = new BigNumber(body.result.ledger.total_coins).div(100000000);
          return cb(supply);
        } else {
          console.log("Data not available. Data:", body);
          return cb(false);
